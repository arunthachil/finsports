...
window.gameprompt = function(result) {
    if (result === null) {
      	toastr.info("Game adding cancelled");
    } else {
      	gameapp.addGame();
  ...
  // ----------------
  (function() {
    $('#gamepromptbutton').on('click', function() {
    	bootbox.dialog({
        title: "Add Game",
        message: '<form class="form-horizontal">' +
          '<div class="form-group">' +
          '<div class="col-md-12">' +
          '<input type="text" class="form-control input-md" id="name" name="name" v-model="game.title" placeholder="Game title"> ' +
          '</div>' +
          '</div>' +
          '</form>',
        buttons: {
  ...
  (function() {
    ...
    $('#gameeditmodal').on('hidden.bs.modal', function() {
        $('.error').html('');
        gameapp.game.title = '';
      	gameapp.game.gameId = '';
    });
